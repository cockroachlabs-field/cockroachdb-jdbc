...
public interface RetryListener {
     ...
     */
    default void beforeRetry(String methodName, int attempt, SQLException ex, Duration executionTime) {
    }

    /**
     * Invoked at the end of a transaction retry attempt, whether it succeeded or failed.
     *
     * @param methodName the original JDBC method that threw a retryable exception
     * @param attempt the current attempt number, 1-based
     * @param ex a new SQL exception from the last retry attempt if any, or null if the retry was successful
     * @param executionTime total time spent in retry attempts
     */
